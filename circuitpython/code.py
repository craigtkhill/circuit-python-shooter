...
acceleration_peak = 9.81 # variable containing the maximum acceleration values
...
controller = ControllerLightPatterns(0.1) # controller object initalised with brightness of 10%
...
while True:
    if cp.switch: # if the cp switch is set to the true position
        ...
        if abs(y) <= acceleration_peak: # if the absolute values of y is less than the acceleration peak
            if y < -3: # and acceleration y is less than -3
                kbd.press(Keycode.DOWN_ARROW) # the down arrow is pressed
                kbd.release(Keycode.UP_ARROW) # and the up arrow is released if it is down
            elif y > 3: # else if the acceleration is greater than 3
                kbd.press(Keycode.UP_ARROW) # the up arrow is pressed
                kbd.release(Keycode.DOWN_ARROW) # and the down arrow is released if it is down
        ...
        if cp.button_a: # if the button a is pressed
            if bullet_fired: # if the bullet has been fired
